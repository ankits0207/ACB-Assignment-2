...
cut_cost_str = raw_input('Enter the cut cost array as a space separated string: ')
length_str_arr = length_str.split()
cost_str_arr = cost_str.split()
cost_arr = []
cut_cost_str_arr = cut_cost_str.split()
cut_cost_arr = []

for s in cost_str_arr:
    cost_arr.append(int(s))

for s in cut_cost_str_arr:
    cut_cost_arr.append(int(s))
...
def get_price(my_list):
    ...
    for e in my_list:
        ...
        price += cost_arr[index] - cut_cost_arr[index]
    return price
...
while i < len(length_str_arr) and j <= len(length_str_arr):
    if matrix_list_1[j] < matrix_list_2[i]:
        ...
        val, idx = get_max(my_matrix[i - 1][j].value, my_matrix[i][j - matrix_list_2[i]].value + cost_arr[i] - cut_cost_arr[i])
